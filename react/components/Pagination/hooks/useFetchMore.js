...
function reducer(state, action) {
  const { maxItemsPerPage, to, from, rollbackState, pageChoose } = action.args
  ...
  switch (action.type) {
    ...
    case 'CHOOSE_PAGE':
      return {
        ...state,
        //previousPage: pageChoose - 1,
        previousPage: pageChoose - 1,
        page: pageChoose,
        //nextPage: pageChoose + 1,
        nextPage: pageChoose + 1,
        from,
        to,
      }
...
const handleFetchMore = async (
  ...
  const isChoose  = direction === FETCH_TYPE.CHOOSE
...
export const useFetchMore = props => {
  ...
  const totalPages  = Math.ceil(totalNumberProducts / maxItemsPerPage)
  ...
  const handleFetchChoosePage = async (e) => {
    const rollbackState = pageState
    const pageChoose    = e
    const to            = (e * maxItemsPerPage) - 1
    const from          = max(0, to - maxItemsPerPage + 1)

    setInfiniteScrollError(false)
    pageDispatch({ type: 'CHOOSE_PAGE', args: { to, from, pageChoose } })
